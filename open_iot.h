...
#define OPEN_IOT_RETURN_CODES(m)            \
    ...
    m(OPEN_IOT_AES_INVALID_BLOCK_SIZE,109U) \
    m(OPEN_IOT_AES_ENCODE_FAILED,110U)      \
...
struct open_iot_config
{
    ...
    uint32_t key_exchange_complete;
};
...
struct open_iot {
    ...
    uint32_t (*set_key_exchange_complete) (struct open_iot_config*, uint32_t state);
...
uint32_t open_iot_set_key_exchange_complete(struct open_iot*, uint32_t value);
